...
import { Target, Users, Globe, Envelope } from "@phosphor-icons/react";
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
          ...
          <div className="lg:col-span-7">
            <motion.div variants={containerVariants} initial="hidden" whileInView="show" viewport={{ once: true, margin: "-100px" }} className="space-y-8 text-lg md:text-xl text-gray-300 font-light leading-relaxed">
              <motion.div variants={itemVariants} className="flex gap-4 items-start">
                <Globe className="w-8 h-8 text-brand-blue shrink-0 mt-1" weight="duotone" />
                <p>{t("p1")}</p>
              </motion.div>
              
              <motion.div variants={itemVariants} className="flex gap-4 items-start">
                <Target className="w-8 h-8 text-brand-blue shrink-0 mt-1" weight="duotone" />
                <p className="text-white font-medium">{t("p2")}</p>
              </motion.div>
              
              <motion.div variants={itemVariants} className="flex gap-4 items-start">
                <Users className="w-8 h-8 text-brand-blue shrink-0 mt-1" weight="duotone" />
                <p>{t("p3")}</p>
              </motion.div>

              <motion.div variants={itemVariants} className="flex gap-4 items-start pt-6 border-t border-gray-800">
                <Envelope className="w-6 h-6 text-gray-400 shrink-0 mt-1" weight="duotone" />
