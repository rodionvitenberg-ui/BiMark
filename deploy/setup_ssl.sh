...
# ==================================================================
#   sudo bash /var/www/bimark/deploy/setup_ssl.sh
#
# Shared domain cert covers both /gardenhouse and /bimark.
#
...
# Re-upsert bimark locations in case certbot rewrote conf
if [ -f "${APP_DIR}/deploy/nginx/bimark.locations.conf" ]; then
    ensure_bimark_nginx \
        "${APP_DIR}/deploy/nginx/bimark.locations.conf" \
        "${APP_DIR}/deploy/nginx/maintest.site.conf" || true
fi
